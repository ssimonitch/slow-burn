...
import { companionStateUpdateSchema, workoutSetInsertSchema, workoutSessionInsertSchema } from './storage';
...
describe('storage schemas', () => {
  ...
  it('enforces non-negative affinity updates', () => {
    expect(() => companionStateUpdateSchema.parse({ affinity_xp: -1, level: 1 })).toThrow();
  });
});
